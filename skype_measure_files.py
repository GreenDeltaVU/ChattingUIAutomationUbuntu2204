...
def get_skype_pid_with_highest_resource_usage():
    skype_processes = []

    # Iterate through all running processes
    for process in psutil.process_iter(attrs=["pid", "name", "memory_percent"]):
        if (
            "skype" in process.info["name"].lower()
        ):  # Check if process name contains "skype"
    ...
    highest_cpu_usage_process = max(
        skype_processes, key=lambda process: process.info["memory_percent"]
    )

    return highest_cpu_usage_process.info["pid"]


def send_a_message_skype(message):
    global measurement_running
    time.sleep(5)
    print("going to click chat")
    # Click on the chat button to open the chat window
    pyautogui.click(x=211, y=515)  # Replace with the actual coordinates
    print("chat-clicked")

    time.sleep(2)
    # Click here to write the message
    pyautogui.click(x=766, y=1036)  # Replace with the actual coordinates
...
def send_an_image_skype():
    ...
    pyautogui.click(x=1468, y=1032)  # Replace with the actual coordinates
...
def send_a_pdf_skype():
    global measurement_running
    time.sleep(5)
    print("going to click chat")
    # Click on the chat PERSON
    pyautogui.click(x=211, y=470)  # Replace with the actual coordinates
    print("chat-clicked")
    ...
    time.sleep(2)
    # Click here to select pdfs folder
    pyautogui.click(x=614, y=657)  # Replace with the actual coordinates
...
def powerjoular_measurement_function(p_id):
    global measurement_running
    powerjoular_command = (
        f'echo " " | sudo -S -k powerjoular -l -p {p_id} -f skype_send_pdf_energy.csv'
    )
...
if __name__ == "__main__":
    ...
    for i in range(2):
        ...
        time.sleep(5)
        # Get the skype PID and start powerjoular measurement
        p_id = get_skype_pid_with_highest_resource_usage()

        if p_id is not None:
            print(
                f"skype meeting with the highest resource consumption is running with PID: {p_id}"
            )
        ...
        # time.sleep(2)
        print(f"skype process started with PID: {p_id}")
